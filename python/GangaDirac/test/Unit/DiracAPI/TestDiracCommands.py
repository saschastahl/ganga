...
def dirac_job():
    ...
    confirm = execute(final_submit_script, cred_req=cred_req)
    ...
    status = execute('status([%s], %s)' % (job_id, repr(statusmapping)), cred_req=cred_req)
    ...
    output_data_info = execute('getOutputDataInfo("%s")' % job_id, cred_req=cred_req)
    ...
    confirm = execute('removeFile("%s")' % get_file_lfn, cred_req=cred_req)
...
class TestDiracCommands(object):
    def test_peek(self, dirac_job):
        confirm = execute('peek("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_getJobCPUTime(self, dirac_job):
        confirm = execute('getJobCPUTime("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_getOutputData(self, dirac_job):
        confirm = execute('getOutputData("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_getOutputSandbox(self, dirac_job):
        confirm = execute('getOutputSandbox("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_getOutputDataInfo(self, dirac_job):
        confirm = execute('getOutputDataInfo("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert isinstance(confirm['getFile.dst'], dict), 'Command not executed successfully'

    def test_getOutputDataLFNs(self, dirac_job):
        confirm = execute('getOutputDataLFNs("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_normCPUTime(self, dirac_job):
        confirm = execute('normCPUTime("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        assert isinstance(confirm, str), 'Command not executed successfully'

    def test_getStateTime(self, dirac_job):
        confirm = execute('getStateTime("%s", "completed")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert isinstance(confirm, datetime.datetime), 'Command not executed successfully'

    def test_timedetails(self, dirac_job):
        confirm = execute('timedetails("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert isinstance(confirm, dict), 'Command not executed successfully'

    def test_y_reschedule(self, dirac_job):
        confirm = execute('reschedule("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_z_kill(self, dirac_job):
        # remove_files()
        confirm = execute('kill("%s")' % dirac_job.id, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_status(self, dirac_job):
        confirm = execute('status([%s], %s)' % (dirac_job.id, repr(statusmapping)), cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert isinstance(confirm, list), 'Command not executed successfully'

    def test_getFile(self, dirac_job):
        confirm = execute('getFile("%s")' % dirac_job.get_file_lfn, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_removeFile(self, dirac_job):
        confirm = execute('removeFile("%s")' % dirac_job.remove_file_lfn, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_ping(self, dirac_job):
        confirm = execute('ping("WorkloadManagement","JobManager")', cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_getMetadata(self, dirac_job):
        confirm = execute('getMetadata("%s")' % dirac_job.get_file_lfn, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_getReplicas(self, dirac_job):
        confirm = execute('getReplicas("%s")' % dirac_job.get_file_lfn, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_a_replicateFile(self, dirac_job):
        new_location = 'UKI-SOUTHGRID-OX-HEP-disk'
        confirm = execute('replicateFile("%s","%s","")' % (dirac_job.get_file_lfn, new_location), cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_b_removeReplica(self, dirac_job):
        new_location = 'UKI-SOUTHGRID-OX-HEP-disk'
        confirm = execute('removeReplica("%s","%s")' % (dirac_job.get_file_lfn, new_location), cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    def test_splitInputData(self, dirac_job):
        confirm = execute('splitInputData("%s","1")' % dirac_job.get_file_lfn, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'
    ...
    def test_uploadFile(self, dirac_job):
        ...
        confirm = execute('uploadFile("%s","upload_file","%s")' % (new_lfn, location), cred_req=dirac_job.cred_req)
        assert isinstance(confirm, dict), 'Command not executed successfully'
        confirm_remove = execute('removeFile("%s")' % new_lfn, cred_req=dirac_job.cred_req)
    ...
    def test_addFile(self, dirac_job, tmpdir):
        ...
        confirm = execute('addFile("%s","%s","%s","")' % (new_lfn, temp_file, location), cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'
        confirm_remove = execute('removeFile("%s")' % new_lfn, cred_req=dirac_job.cred_req)
        logger.info(confirm)
    ...
    def test_getJobGroupJobs(self, dirac_job):
        confirm = execute('getJobGroupJobs("")', cred_req=dirac_job.cred_req)
        assert confirm['OK'], 'Command not executed successfully'
...
# LHCb commands:
    ...
    def test_bkQueryDict(self, dirac_job):
        confirm = execute('bkQueryDict({"FileType":"Path","ConfigName":"LHCb","ConfigVersion":"Collision09","EventType":"10","ProcessingPass":"Real Data","DataTakingConditions":"Beam450GeV-VeloOpen-MagDown"})', cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    @pytest.mark.skip(reason='Should be moved to LHCb')
    def test_checkSites(self, dirac_job):
        confirm = execute('checkSites()', cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    @pytest.mark.skip(reason='Should be moved to LHCb')
    def test_bkMetaData(self, dirac_job):
        confirm = execute('bkMetaData("")', cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    @pytest.mark.skip(reason='Should be moved to LHCb')
    def test_getDataset(self, dirac_job):
        confirm = execute('getDataset("LHCb/Collision09/Beam450GeV-VeloOpen-MagDown/Real Data + RecoToDST-07/10/DST","","Path","","","")', cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'

    @pytest.mark.skip(reason='Should be moved to LHCb')
    def test_checkTier1s(self, dirac_job):
        confirm = execute('checkTier1s()', cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['OK'], 'Command not executed successfully'
...
# Problematic tests
    ...
    def test_getInputDataCatalog(self, dirac_job):
        confirm = execute('getInputDataCatalog("%s","","")' % dirac_job.get_file_lfn, cred_req=dirac_job.cred_req)
        logger.info(confirm)
    ...
    def test_getLHCbInputDataCatalog(self, dirac_job):
        confirm = execute('getLHCbInputDataCatalog("%s",0,"","")' % dirac_job.get_file_lfn, cred_req=dirac_job.cred_req)
        logger.info(confirm)
        assert confirm['Message'] == 'Failed to access all of requested input data', 'Command not executed successfully'
